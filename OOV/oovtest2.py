...
#hmm字典
dic=codecs.open('dict.merge.20170515.list.cn','r','utf-8')
...
def hmm(data):
	...
	while i<length:
		...
		if i+2<length:
			possible_word=sen2[i:i+2]
			if possible_word in char_2:
				temp.append(possible_word)
				i+=2
				continue

		possible_word=sen2[i]
		temp.append(possible_word)
		i+=1
...
if __name__ == "__main__":	
	...
	for line in sen:
		...
		length=len(testsen1)
		if length>0:
			line_new=[0 for x in range(0,length)]
			line_new[0]=testsen1[0]
			line_new1=[]
			k=1
			m=0
			for k in range(1,length):
				if not check_chinese(testsen1[k]):
					m+=1
					line_new[m]=testsen1[k]
				else:
					if not check_indict(testsen1[k]) or (check_indict(testsen1[k]) and (not check_indict(testsen1[k-1])) and check_chinese(testsen1[k-1])):
						line_new[m]=line_new[m]+testsen1[k]
					else:
						m+=1
						line_new[m]=testsen1[k]
			line_new1=line_new[0:m+1]
			#print (' '.join(line_new1))	
			# outresult1=' '.join(line_new1)
			# segResult.write(outresult1+'\n')
			#sen2=''.join(line_new1)
			#testsen2=sen2.split()
			testsen2=line_new1
			length2=len(testsen2)
			outresult= []
			for j in range(0,length2):
				if not check_chinese(testsen2[j]):   #判断是不是中文，不是中文直接输出
					outresult.append(testsen2[j])
				else:
					if check_indict(testsen2[j]):
						outresult.append(testsen2[j])
					else:
						newoov=testsen2[j]
						newoov1=''.join(newoov)
